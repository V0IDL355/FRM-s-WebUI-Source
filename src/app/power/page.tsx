...
const chartConfig = {
    powercapacity: {
        label: "PowerCapacity", color: "#ca9ee6",
    }, powerproduction: {
        label: "PowerProduction", color: "#e78284",
    }, powerconsumed: {
        label: "PowerConsumed", color: "#ef9f76",
    }, powermaxconsumed: {
        label: "PowerMaxConsumed", color: "#e5c890",
    },
...
function Droppable(props: any) {
    ...
    return (<div
            ...
            style={isOver ? {
                backgroundColor: "hsla(220, 14%, 27%, 0.2)", borderColor: "hsl(220, 14%, 27%)",
            } : {}}
        >
            {props.children}
        </div>);
...
export default function Power() {
    ...
    const selectedRowIds: string[] = Object.keys(rowSelection).filter((id: string) => rowSelection[id],);
    ...
    return (<div style={{margin: 5, padding: 25}}>
            ...
            >
                <ChartContainer
                    style={{
                        height: "20vh", width: "100%", padding: 10, justifyContent: "center",
                    }}
                    config={chartConfig}
                >
                    ...
                    <DndContext
                        onDragEnd={(event) => {
                            ...
                            if (over) {
                                console.table(items);
                                setItems((prev) => {
                                    const origin = Object.keys(prev).find((key) => prev[key as keyof typeof prev].includes(active.id),) as keyof typeof prev;
                        ...
                        <div className={"flex w-full"}>
                            <div className={"grid grid-rows-2 grid-cols-4 gap-1 flex-grow"}>
                                {Object.entries(items)
                                    ...
                                    .map(([priority, itemList]) => (<Card key={priority}>
                                            <CardHeader>
                                                <CardTitle>
                                                    {priority
                                                        .replace("-", " ")
                                            ...
                                            <CardContent>
                                                <Droppable id={priority}>
                                                    {itemList.map((item) => (<Draggable key={item} id={item}>
                                                            {item}
                                                        </Draggable>))}
                                                </Droppable>
                                            </CardContent>
                            ...
                            <Card className={"ml-[5px]"}>
                                ...
                                <CardContent>
                                    <Droppable id={"undefined"}>
                                        {items["undefined"] && items["undefined"].map((item) => (
